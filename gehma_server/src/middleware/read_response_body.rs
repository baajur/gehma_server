    ...
    fn poll(&mut self) -> Poll<Self::Item, Self::Error> {
        ...
        Ok(Async::Ready(res.map_body(move |_, body| {
            ResponseBody::Body(BodyLogger {
                url: self.url.clone(), //TODO don't need clone here
                body,
...
pub struct BodyLogger<B> {
    url: String, //TODO don't need ownership here
    body: ResponseBody<B>,
    body_accum: BytesMut,
}
...
impl<B> Drop for BodyLogger<B> {
    fn drop(&mut self) {
        if self.url != "/" {
            //We don't want to print html pages
            //We are only interested in api responses 
            debug!("body: {:#?}", self.body_accum);
        }
    }
}
